  ...
  async execute(interaction, client) {
      ...
      interaction.reply({
        embeds: [
          embed
            .setColor(mConfig.errorEmbedColor)
            .setDescription(
              `${mConfig.error} **The role you are trying to manage is higher than mine.**`
            ),
        ],
        ephemeral: true,
      });
    ...
    switch (subCommand) {
      case "add":
        embed
          .setDescription(
            `${mConfig.success} Added role \`${role.name}\` to ${user.username}`
          )
      ...
      case "remove":
        embed
          .setDescription(
            `${mConfig.success} Removed role \`${role.name}\` from ${user.username}`
          )
          .setColor(mConfig.successEmbedColor);
