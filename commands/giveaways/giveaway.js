...
module.exports = {
  data: new SlashCommandBuilder()
      ...
      subcommand
        ...
        .addChannelOption((option) =>
          option
            .setName("channel")
            .setDescription(
              "Specify the channel where to send the giveaway. (default is current channel)"
            )
            .setRequired(false)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("pause")
        .setDescription("️Pauses a giveaway.")
        .addStringOption((option) =>
          option
            .setName("message-id")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("end")
        .setDescription("️Ends a giveaway.")
        .addStringOption((option) =>
          option
            .setName("message-id")
            .setDescription("Specify giveaway message-id.")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("reroll")
        .setDescription("️Selects a new giveaway winner.")
        .addStringOption((option) =>
          option
            .setName("message-id")
            .setDescription("Specify giveaway message-id.")
  ...
  async execute(interaction, client) {
    ...
    const errorEmbed = new EmbedBuilder().setColor(
      client.messageConfig.errorEmbedColor
    );
    const successEmbed = new EmbedBuilder().setColor(
      client.messageConfig.successEmbedColor
    );
    ...
    if (sub === "start") {
      ...
      if (isNaN(duration)) {
        errorEmbed.setDescription(
          "Enter the corredct giveaway length format! `1d, 1h, 1m, 1s`!"
        );
    ...
    if (!giveaway) {
      errorEmbed.setDescription(
        `Giveaway with ID ${messageid} was not found in the database!`
      );
      return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
    }
    ...
    if (sub === "pause") {
      ...
      await client.giveawaysManager
        ...
        .then(() => {
          successEmbed
            .setDescription("⏸️ The giveaway has been paused!")
            .setColor("Blue");
          return interaction.reply({ embeds: [successEmbed], ephemeral: true });
        })
        .catch((err) => {
    ...
    if (sub === "unpause") {
      client.giveawaysManager
        ...
        .then(() => {
          successEmbed
            .setDescription("▶️ The giveaway has been unpaused!")
            .setColor("Blue");
          return interaction.reply({ embeds: [successEmbed], ephemeral: true });
        })
        .catch((err) => {
    ...
    if (sub === "end") {
      client.giveawaysManager
        ...
        .then(() => {
          successEmbed
            .setDescription("▶️ The giveaway has been stopped!")
            .setColor("Blue");
          return interaction.reply({ embeds: [successEmbed], ephemeral: true });
        })
        .catch((err) => {
    ...
    if (sub === "reroll") {
      await client.giveawaysManager
        ...
        .then(() => {
          successEmbed
            .setDescription("🎉 The giveaway has a new winner!")
            .setColor("Gold");
          return interaction.reply({ embeds: [successEmbed], ephemeral: true });
        })
        .catch((err) => {
